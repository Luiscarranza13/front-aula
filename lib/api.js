...
export const login = async (email, password) => {
  ...
  const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Tiempo de espera agotado. Verifica tu conexión a internet o el estado del servidor.')), timeoutMs));

  const { data, error } = await Promise.race([authPromise, timeoutPromise]);
  
  if (error) throw new Error(mapError(error, 'No se pudo iniciar sesión'));

  const userId = data.user.id;
  const profilePromise = supabase.from('usuarios').select('*').eq('id', userId).maybeSingle();
...
// Cache ultra veloz para SPA Navigation
const apiCache = {
  store: new Map(),
  get: (key, ttl = 30000) => {
    const cached = apiCache.store.get(key);
    if (cached && Date.now() - cached.time < ttl) return cached.data;
    return null;
  },
  set: (key, data) => apiCache.store.set(key, { data, time: Date.now() }),
  invalidate: (prefix) => {
    for (const key of apiCache.store.keys()) {
      if (key.startsWith(prefix)) apiCache.store.delete(key);
    }
  }
};

// Utilidad interna: Obtener suscripciones del estudiante
const getStudentEnrollmentIds = async (userId) => {
  // Intentamos la tabla en español 'inscripciones' primero
  const { data: inscData, error: inscError } = await supabase.from('inscripciones').select('id_curso').eq('id_estudiante', userId);
  if (!inscError && inscData) return inscData.map(e => e.id_curso);
  
  // Fallback a 'enrollments' tabla en inglés
  const { data: enrData, error: enrError } = await supabase.from('enrollments').select('course_id').eq('student_id', userId);
  if (!enrError && enrData) return enrData.map(e => e.course_id);
...
export const getCourses = async () => {
  ...
  const cacheKey = `courses:all:${role}:${user.id}`;
  const cached = apiCache.get(cacheKey);
  if (cached) return cached;

  let query = supabase.from('cursos').select('*').order('creado_en', { ascending: false });

  // IMPLEMENTACIÓN RBAC
  if (role === 'docente' || role === 'profesor') {
    // Si es docente, mostrar solos los suyos
    query = query.eq('id_docente', user.id);
  } else if (role === 'estudiante') {
    // Si es estudiante, obtener ids de inscripciones
    const enrolledIds = await getStudentEnrollmentIds(user.id);
    if (enrolledIds.length > 0) {
      // Filtrar cursos por los ids permitidos
      query = query.in('id', enrolledIds);
    } else {
      // No está inscrito en nada, retornamos vacío de inmediato
      const result = [];
  ...
  const teacherMap = await fetchUsersMap((data ?? []).map((row) => row.id_docente));
  const result = (data ?? []).map((row) => normalizeCourse(row, teacherMap));
...
export const getCourse = async (id) => {
  ...
  // Si es estudiante, verificamos que esté inscrito antes de cargar el curso
  if (role === 'estudiante') {
    const enrolledIds = await getStudentEnrollmentIds(user.id);
    if (!enrolledIds.includes(toNumber(id))) {
  ...
  // Si es docente, verificamos que él dicte el curso (o sea admin)
  if ((role === 'docente' || role === 'profesor') && data.id_docente !== user.id) {
     throw new Error("No tienes permisos para ver un curso que no dictas.");
  }
...
export const createCourse = async (courseData) => {
  ...
  const payload = stripUndefined({
    ...
    id_docente: courseData.id_docente ?? courseData.docenteId ?? defaultTeacherId,
...
export const getAllTasks = async () => {
  ...
  if (!user) return [];
  const role = user.rol || 'estudiante';
  const cacheKey = `tasks:all:${role}:${user.id}`;
  ...
  // Aplicar RBAC: Obtener cursos a los que tiene acceso el usuario
  const allowedCourses = await getCourses();
  const allowedCourseIds = new Set(allowedCourses.map(c => c.id));

  // Filtrar tareas solo a cursos permitidos
  const validTasksData = (data ?? []).filter(row => {
    const cid = toNumber(row.id_curso ?? row.course_id ?? row.cursoId);
    return allowedCourseIds.has(cid) || role === 'admin';
  });

  const courseMap = await fetchCoursesMap(validTasksData.map((row) => row.id_curso ?? row.course_id ?? row.cursoId));
  const result = validTasksData.map((row) => normalizeTask(row, courseMap));
...
export const getExams = async (courseId) => {
  ...
  if (!user) return [];
  const role = user.rol || 'estudiante';
  const cacheKey = `exams:all:${role}:${user.id}:${courseId || 'all'}`;
  ...
  // Aplicar RBAC: Obtener cursos permitidos
  const allowedCourses = await getCourses();
  const allowedCourseIds = new Set(allowedCourses.map(c => c.id));

  // Filtrar exámenes solo a cursos permitidos
  const validExamsData = (data ?? []).filter(row => {
    const cid = toNumber(row.id_curso ?? row.course_id ?? row.cursoId);
    return allowedCourseIds.has(cid) || role === 'admin';
  });

  const examIds = validExamsData.map((row) => toNumber(row.id));
  const courseMap = await fetchCoursesMap(validExamsData.map((row) => row.id_curso));
  ...
  const result = validExamsData.map((row) => normalizeExam({ ...row, preguntas: questionsByExam.get(toNumber(row.id)) ?? [] }, courseMap));
...
export const getDashboardStats = async () => {
  const cached = apiCache.get('dashboard:stats', 20000); // 20s ultra-cache
  if (cached) return cached;
  ...
  apiCache.set('dashboard:stats', result);
  return result;
};
